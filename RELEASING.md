...
The packages are consumed by TrustPlatform (Tessio Cloud) and by Tessio Labs, both via
...
- **TrustPlatform**: `src/tessio/Tessio.Verification/Tessio.Verification.csproj`, the
...
- **Tessio Labs**: `services/verifier/Verifier.csproj` if it consumes the changed package.

## Co-developing before a release

You do not have to publish to test a consumer against an unreleased change. In TrustPlatform:

```sh
export TessioVerifierSource=~/Work/Tessio/tessio-verifier
