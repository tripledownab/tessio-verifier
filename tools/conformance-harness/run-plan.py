...
def record_run(path, plan_id, plan, module_count):
    ...
    """
    root = git(None, "rev-parse", "--show-toplevel")

    # Refuse a dirty src/, which is the one way this record could lie without anyone meaning to: the
    # harness builds from the working tree, so uncommitted source would be what the suite actually
    # tested while HEAD:src names something else entirely.
    if git(root, "status", "--porcelain", "--", "src"):
        sys.exit("src/ has uncommitted changes, so the suite did not test what HEAD:src names. "
                 "Commit first, re-run, then record.")
    ...
    document.setdefault("runs", {})[variant] = {
        "srcTree": git(root, "rev-parse", "HEAD:src"),
...
def main():
    ...
    if args.record:
        record_run(args.record, args.plan_id, plan, len(modules))
